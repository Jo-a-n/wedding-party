...
- [ ] Add a simple cooldown (e.g. 200-300ms debounce) on `createBurst` to prevent spam
- [ ] This is client-side only — not a security measure, just prevents accidental rapid-fire

### 9. Make wish submission truly optimistic

`app/components/wish-input.tsx:75-86`

- [ ] Generate a temporary ID client-side and call `onOptimisticAdd` immediately before the DB insert
- [ ] On success, reconcile the temp ID with the real one via the realtime subscription
- [ ] On failure, remove the optimistic wish and show error

### 10. Reuse FFmpeg instance across batch

`lib/media-utils.ts:125-163`

- [ ] Refactor `transcodeMovToMp4` to optionally accept an existing FFmpeg instance
- [ ] In `gallery-upload.tsx`, create one instance for the batch and pass it to each call
- [ ] Terminate after the batch completes
